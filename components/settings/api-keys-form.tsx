...
export function ApiKeysForm({ isProPlan }: { isProPlan: boolean }) {
  ...
  const handleModelChange = (modelId: string) => {
    ...
    if (!isProPlan) {
      const hasRequiredKey = apiKeys.some(
        (k) => k.service === selectedModel.provider,
      );
      if (!hasRequiredKey) {
        toast.error(
          `Please add your ${selectedModel.provider === "openai" ? "OpenAI" : "Anthropic"} API key first`,
        );
        return;
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-xl bg-gradient-to-br from-white/50 to-white/30 border border-white/40 shadow-xl backdrop-blur-sm">
        <Label className="text-gray-800 font-semibold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
          Default AI Model
        </Label>
          ...
          <SelectContent>
            {AI_MODELS.map((model) => (
              ...
              >
                ...
                {!isModelSelectable(model.id) && (
                  <span className="ml-1.5 text-gray-700">(No API Key set)</span>
                )}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      {/* API Keys Card */}
      <div className="p-5 rounded-xl bg-gradient-to-br from-white/50 to-white/30 border border-white/40 shadow-xl backdrop-blur-sm">
        <Label className="text-gray-800 font-semibold bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text text-transparent">
          API Keys
        </Label>
              ...
              >
                  ...
                  {existingKey && (
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        className="h-7 px-2 text-gray-700 hover:text-gray-900 transition-colors"
                      >
                        {isVisible ? (
                ...
                {providerModels.length > 0 && (
                  <div className="text-xs text-gray-700 mt-2">
                    Available models:{" "}
                    {providerModels
                      .map((m) => `${m.name}${m.unstable ? " (Unstable)" : ""}`)
                      .join(", ")}
                  </div>
                )}
              </div>
