...
export function ModelSelector() {
        ...
        if (storedModel) {
          ...
          if (model && keys.some((k: ApiKey) => k.service === model.provider)) {
            ...
            if (firstAvailableModel) {
              ...
              toast.info(
                `Switched to ${firstAvailableModel.shortName} as previous model was unavailable`,
              );
            } else {
              setDefaultModel("");
  ...
  useEffect(() => {
    ...
    if (
      currentModel &&
      !apiKeys.some((k) => k.service === currentModel.provider)
    ) {
      ...
      if (firstAvailableModel) {
        ...
        localStorage.removeItem(MODEL_STORAGE_KEY);
        toast.info(
          "No AI models available. Please add an API key in settings.",
        );
      }
    }
  ...
  return (
    <div className="relative">
      <Select value={defaultModel} onValueChange={handleModelChange}>
        <SelectTrigger className="h-7 px-2 text-xs border-none bg-transparent hover:bg-purple-500/5 transition-colors">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {!isModelSelectable(model.id) && (
                  <span className="ml-1 text-gray-700">(No API Key)</span>
                )}
              </div>
            </SelectItem>
          ))}
