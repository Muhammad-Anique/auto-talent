...
type Theme = {
  ...
  linkColor?: string;      // Link text
  backgroundColor?: string; // Optional background (if used)
};
...
export function createResumeStyles(
  ...
  const {
    ...
    skills_margin_top,
    skills_margin_bottom,
    skills_margin_horizontal,
    skills_item_spacing,
    experience_margin_top,
    experience_margin_bottom,
    experience_margin_horizontal,
    experience_item_spacing,
    projects_margin_top,
    projects_margin_bottom,
    projects_margin_horizontal,
    projects_item_spacing,
    education_margin_top,
    education_margin_bottom,
    education_margin_horizontal,
    education_item_spacing,
    footer_width,
  } = settings;

  // Adjust sizes based on layout
  const adjustedNameSize = layout === "compact" ? header_name_size - 2 : 
                          layout === "minimal" ? header_name_size + 2 :
                          layout === "executive" ? header_name_size + 4 :
                          layout === "corporate" ? header_name_size + 2 :
                          header_name_size;

  const adjustedFontSize = layout === "compact" ? document_font_size - 1 :
                          layout === "minimal" ? document_font_size + 1 :
                          layout === "executive" ? document_font_size + 1 :
                          document_font_size;

  const fontFamily = layout === "executive" || layout === "corporate" ? "Times-Roman" : "Helvetica";
  const boldFontFamily = layout === "executive" || layout === "corporate" ? "Times-Bold" : "Helvetica-Bold";
  ...
  return StyleSheet.create({
    ...
    name: {
      fontSize: adjustedNameSize,
      fontFamily: boldFontFamily,
      marginBottom: header_name_bottom_spacing,
      color: theme.color || "#111827",
      textAlign: layout === "left-aligned" ? "left" : "center",
    },
    contactInfo: {
      fontSize: adjustedFontSize,
      color: "#374151",
      flexDirection: layout === "left-aligned" ? "column" : "row",
      justifyContent: layout === "left-aligned" ? "flex-start" : "center",
      flexWrap: "wrap",
      gap: layout === "left-aligned" ? 2 : 4,
    },
    sectionTitle: {
      fontSize: adjustedFontSize,
      fontFamily: boldFontFamily,
...
interface ResumePDFDocumentProps {
  ...
  layout?: "classic" | "left-aligned" | "compact" | "modern" | "professional" | "minimal" | "sidebar" | "executive" | "corporate";
}
