...
export async function improveWorkExperience(
  ...
  const { object } = await generateObject({
    ...
    prompt: `Please improve this work experience bullet point while maintaining its core message and truthfulness${
...
export async function improveProject(
  ...
  const { object } = await generateObject({
    ...
    prompt: `Please improve this project bullet point while maintaining its core message and truthfulness${
      customPrompt ? `. Additional requirements: ${customPrompt}` : ""
    }:\n\n"${point}"`,
